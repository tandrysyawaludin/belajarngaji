...
import { getIqraPage, IQRA_JILID_2_PAGES } from "@/lib/iqra";
import { strings } from "@/lib/strings";

export function generateStaticParams() {
  return IQRA_JILID_2_PAGES.map((page) => ({
...
export async function generateMetadata({
  ...
  const page = getIqraPage(2, pageNumber);
...
export default async function IqraJilidTwoDynamicPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
