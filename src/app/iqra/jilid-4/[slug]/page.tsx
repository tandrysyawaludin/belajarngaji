...
import { getIqraPage, IQRA_JILID_4_PAGES } from "@/lib/iqra";
import { strings } from "@/lib/strings";

export function generateStaticParams() {
  return IQRA_JILID_4_PAGES.map((page) => ({
...
export async function generateMetadata({
  ...
  const page = getIqraPage(4, pageNumber);
...
export default async function IqraJilidFourDynamicPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
