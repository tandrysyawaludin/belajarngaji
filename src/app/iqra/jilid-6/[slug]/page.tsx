...
import { getIqraPage, IQRA_JILID_6_PAGES } from "@/lib/iqra";
import { strings } from "@/lib/strings";

export function generateStaticParams() {
  return IQRA_JILID_6_PAGES.map((page) => ({
...
export async function generateMetadata({
  ...
  const page = getIqraPage(6, pageNumber);
...
export default async function IqraJilidSixDynamicPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
