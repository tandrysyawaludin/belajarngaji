...
import { getIqraPage, IQRA_JILID_3_PAGES } from "@/lib/iqra";
import { strings } from "@/lib/strings";

export function generateStaticParams() {
  return IQRA_JILID_3_PAGES.map((page) => ({
...
export async function generateMetadata({
  ...
  const page = getIqraPage(3, pageNumber);
...
export default async function IqraJilidThreeDynamicPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
