...
export function SurahListContent({ surahs }: { surahs: Surah[] }) {
  const strings = useStrings();

  return (
    <div className="flex flex-col gap-5">
      <section className="rounded-3xl bg-white/90 p-5 shadow-md ring-2 ring-pink-100">
        <h1 className="text-2xl font-extrabold text-pink-600 sm:text-3xl">{strings.navSurah}</h1>
        <p className="mt-1 text-base font-semibold text-pink-900/70">{strings.seoSurahListBlurb}</p>
      </section>
      <ContinueReading />
          ...
          return (
            ...
            >
              ...
              <p className="text-xs font-semibold opacity-80">
                {s.meaning} • {s.verses} {strings.versesLabel}
              </p>
            </Link>
          );
        })}
